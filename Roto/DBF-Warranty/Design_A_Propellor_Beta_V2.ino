...
byte const NUM_SENSORS = 25;//number of sensor reads; not number of sensors
...
void setup()
{
  ...
  FastLED.addLeds<LED_TYPE, DATA_PIN, COLOR_ORDER>(leds, NUM_LEDS * 4).setCorrection(TypicalLEDStrip); // tell FastLED about the LED strip configuration
...
void lightLED() {
  for (i = 0; i < ledItr; i++) {
    for (j = 0; j < LED_GROUP_SIZE; j++){
      leds[(i * LED_GROUP_SIZE) + j] = ON(i); //turn on leds
    }
  }
  for (i = ledItr; i < NUM_LEDS; i++) {
    for (j = 0; j < LED_GROUP_SIZE; j++){
      leds[(i * LED_GROUP_SIZE;) + j] = OFF; //turn on leds
    }
  }
  if (ledItr == 0 && buttonState == button_ON ) {
    for (j = 0; j < LED_GROUP_SIZE; j++){
      leds[j] = ON(0); //turn on leds
    }
  }
  FastLED.show();
