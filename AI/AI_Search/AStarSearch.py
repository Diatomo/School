...
class Agent:
    ...
    def heuristic(self, botWalk):
        '''determines heuristic for fastest robot'''
        minimum = 10
        for i in range(len(botWalk)):
            if (minimum > robots[botWalk[i]]):
                minimum = robots[botWalk[i]]
        return minimum
    ...
    def genTree(self, parent):
        ...
        if (pPos == parent.left):
            ...
            for i in range(len(pPos)-2):
                for j in range(i+1,len(pPos)-1):
                    ...
                    parent.children.put((self.pathCost(botWalk),self.pathCost(botWalk) + parent.cost + self.heuristic(botWalk),node))
        ...
        #pPack Right Decision Nodes
            ...
            for i in range(len(pPos)-1):
                    ...
                    parent.children.put((self.pathCost(botWalk), node),self.pathCost(botWalk) + parent.cost + self.heuristic(botWalk),node)
                    parent.nChild.append(node)
                    #cleanup
                    botWalk = ""
                    tempL = parent.left
